...
from .prompts import EXTRACT_FROM_FILING_PROMPT, EXTRACT_FROM_WEB_PROMPT
...
class StructuredExtractor:
    ...
    async def extract(
        ...
        schema_web = await self._extract_web_pass(web_text) if web_text else {}
    ...
    async def _extract_web_pass(self, web_text: str) -> dict:
        parsed = await self._llm_extract(
            text=web_text,
            prompt=EXTRACT_FROM_WEB_PROMPT,
            pass_name="web",
        )
        if not parsed:
            return {}
